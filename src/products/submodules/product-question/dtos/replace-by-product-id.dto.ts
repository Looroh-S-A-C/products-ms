import { IsString, IsNotEmpty, IsUUID, IsArray, ValidateNested, ArrayMinSize } from 'class-validator';
import { Type } from 'class-transformer';
import { IsOptional, IsInt, Min, IsEnum } from 'class-validator';
import { QuestionProductType } from '@prisma/client';
import { CreateProductQuestionWithoutProductDto } from './product-question-data.dto';

/**
 * DTO for replacing all product-question relationships
 */
export class ReplaceByProductIdDto {
  /**
   * Product ID
   */
  @IsString()
  @IsNotEmpty()
  @IsUUID()
  productId: string;

  /**
   * Array of questions to replace existing ones
   */
  @IsArray()
  @ArrayMinSize(0, { message: 'Questions array can be empty' })
  @ValidateNested({ each: true })
  @Type(() => CreateProductQuestionWithoutProductDto)
  questions: CreateProductQuestionWithoutProductDto[];
}
